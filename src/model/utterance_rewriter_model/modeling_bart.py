...
class DecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.encoder_attn_layer_norm2 = LayerNorm(self.embed_dim * 2)
        self.fc1 = nn.Linear(self.embed_dim * 2, config.decoder_ffn_dim)
    ...
    def forward(
        ...
        decoder_inputs_M, self_attn_weights = self.self_attn(
            query=x,
            key=x,
        ...
        decoder_inputs_M = F.dropout(decoder_inputs_M, p=self.dropout, training=self.training)
        decoder_inputs_M = residual + decoder_inputs_M
        if not self.normalize_before:
            decoder_inputs_M = self.self_attn_layer_norm(decoder_inputs_M)

        # Cross-Attention Block
        residual = decoder_inputs_M
        assert self.encoder_attn.cache_key != self.self_attn.cache_key
        if self.normalize_before:
            decoder_inputs_M = self.encoder_attn_layer_norm(decoder_inputs_M)
        decoder_inputs_query, cross_attn_weights_query = self.encoder_attn(
            query=decoder_inputs_M,
            key=encoder_hidden_states,
            key_padding_mask=encoder_attn_mask,
            layer_state=layer_state,  # mutates layer state
            output_attentions=output_attentions,
        )
        decoder_inputs_query = residual + decoder_inputs_query

        decoder_inputs_content, cross_attn_weights_content = self.encoder_attn(
            query=decoder_inputs_M,
            key=encoder_hidden_states,
            key_padding_mask=encoder_attn_mask,
            layer_state=layer_state,  # mutates layer state
        ...
        decoder_inputs_content = residual + decoder_inputs_content

        decoder_inputs = torch.cat([decoder_inputs_query, decoder_inputs_content], dim=-1)

        decoder_inputs = F.dropout(decoder_inputs, p=self.dropout, training=self.training)
        if not self.normalize_before:
            decoder_inputs = self.encoder_attn_layer_norm2(decoder_inputs)

        # Fully Connected
        if self.normalize_before:
            decoder_inputs = self.final_layer_norm(decoder_inputs)
        decoder_inputs = self.activation_fn(self.fc1(decoder_inputs))
        decoder_inputs = F.dropout(decoder_inputs, p=self.activation_dropout, training=self.training)
        decoder_inputs = self.fc2(decoder_inputs)
        decoder_inputs = F.dropout(decoder_inputs, p=self.dropout, training=self.training)
        if not self.normalize_before:
            decoder_inputs = self.final_layer_norm(decoder_inputs)
        return (
            decoder_inputs,             # finally output
            decoder_inputs_query,       # query cross-attention output
            decoder_inputs_content,     # content cross-attention output
            decoder_inputs_M,           # self-attention output
            layer_state,
            # self_attn_weights,
            # layer_state,
            # cross_attn_weights_query,
            # cross_attn_weights_content,
...
class BartDecoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding):
        ...
        self.layer_norm = LayerNorm(config.d_model) if config.add_final_layer_norm else None
        self.distribute_layer = DistributeLayer(config)
    ...
    def forward(
        ...
        **unused,
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            x, decoder_inputs_query, decoder_inputs_content, decoder_inputs_M, layer_past = decoder_layer(
                x,
                encoder_hidden_states,
            ...
            if output_attentions:
                all_self_attns += (decoder_inputs_M,)
                all_cross_attentions += (decoder_inputs_content, decoder_inputs_query)

        if self.layer_norm:  # if config.add_final_layer_norm (mBART)
            x = self.layer_norm(x)

        self.distribute_layer(D=x.transpose(0, 1),
                              C_query=decoder_inputs_query.transpose(0, 1),
                              C_content=decoder_inputs_content.transpose(0, 1),
                              M=decoder_inputs_M.transpose(0, 1),
...
class DistributeLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.lambda_dense = nn.Linear(self.hidden_size * 3, 1)   # decoder_output + decoder_content + decoder_query
        self.sigmoid = nn.Sigmoid()
        self.e_dense_layer = nn.Linear(256, self.hidden_size, bias=False)
        self.m_dense_layer = nn.Linear(256, self.hidden_size, bias=False)
    ...
    def forward(self, D, C_query, C_content, M, E, attention_bias_query=None, attention_bias_content=None, inputs=None):
        """
        Args:
        ...
        """
        # calc lambda
        lambda_inputs = torch.cat([D, C_query, C_content], dim=-1)
        lam = self.lambda_dense(lambda_inputs)
        # calc attention
        attention_bias_query = torch.squeeze(attention_bias_query)       # dim = 1, 2
        attention_bias_content = torch.squeeze(attention_bias_content)   # dim = 1, 2
...
class UtteranceRewriterModel(PretrainedBartModel):
    base_model_prefix = "model"
